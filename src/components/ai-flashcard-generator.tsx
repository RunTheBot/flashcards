...
export function AIFlashcardGenerator({ deckId, onCardsGenerated }: AIFlashcardGeneratorProps) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Use AI to automatically generate flashcards on any topic. Just describe what you want to study!
          </DialogDescription>
        </DialogHeader>
...
export function AIFlashcardGeneratorCard({ onTopicSelect, isGenerating = false }: AIFlashcardGeneratorCardProps) {
  const [topic, setTopic] = useState("");
    ...
    <Card>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button type="submit" disabled={!topic.trim() || isGenerating} className="w-full flex items-center gap-2">
            {isGenerating ? (
              <>
                <Loader2 className="h-4 w-4 animate-spin" />
                Creating & Generating...
        ...
        <div className="mt-4 text-sm text-muted-foreground">
          <p>AI will generate a complete set of flashcards on your chosen topic automatically.</p>
        </div>
      </CardContent>
    </Card>
  );
