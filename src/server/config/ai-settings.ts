...
export function getCardCountOptions(): Array<{ value: string; label: string }> {
  ...
  for (let i = settings.flashcards.minCardCount; i <= settings.flashcards.maxCardCount; i += 5) {
 ...
 */
export function getDifficultyOptions(): Array<{ value: string; label: string }> {
  return [
...
export function buildDeckNamePrompt(topic: string): string {
  const settings = getAISettings();
  const examples = settings.prompts.deckName.examples.map(ex => `- ${ex}`).join('\n');
...
export function buildCardCountPrompt(topic: string): string {
  const settings = getAISettings();
  const examples = settings.prompts.cardCount.examples.map(ex => `- ${ex}`).join('\n');
 ...
 */
export function buildFlashcardGenerationPrompt(topic: string, cardCount: number): string {
  const settings = getAISettings();
  const guidelines = settings.prompts.flashcardGeneration.guidelines.map(g => `- ${g}`).join('\n');
